...
class MyTestCase(unittest.TestCase): # test方法的执行顺序按方法名的字母顺序
    ...
    def setUp(self):# 环境预设：打开浏览器访问网页。每个test方法之前都会执行一遍
        ...
        print('1.环境预设')
    @ddt.data(
        {'uname' : 'jackma','pwd' : '123456','expect':'登录成功'},
        {'uname' : 'jackma','pwd' : '1234567','expect':'密码错误'},
        {'uname' : '','pwd' : '123456','expect':'用户名不能为空'}
    )

    @ddt.unpack
    def test_login3(self,uname,expect,pwd): # 测试用例 - 登录
        self.driver.find_element_by_id('accountID').send_keys(uname) # 输入用户名
        self.driver.find_element_by_id('passwordID').send_keys(pwd) # 输入密码
